...
def open_svg(filename, ruta):
    with open(filename + '.svg', 'w', encoding='utf8') as f:
        s = SalidaSVG(f)
        s.cabecera_svg()
        tratar_svg(ruta, s, 20)
        s.cierre_svg()
...
def tratar_svg(ruta, s, x):
    ...
    svg += '<polyline points="' + points + '" fill="none" stroke="green" stroke-width="2" />'
    s.write_svg(svg)
...
def main():
    ...
    for ruta in rutas.findall('ruta'):
        if desiredOutput.upper() == "KML":
            open_kml(fileName + str(i), ruta)
            i += 1
        elif desiredOutput.upper() == "SVG":
            open_svg(fileName + str(i), ruta)
            i += 1
